...
LATEST_BRANCH=git -C $(GAIA_DIR) rev-parse --abbrev-ref HEAD
...
sync-shared:
	...
	# For locale stuff
	cp -r $(GAIA_DIR)/shared/locales/date ./gaia-email/shared/locales/date
	# Shared styles
...
sync-email:
	cp -r $(GAIA_DIR)/apps/email/autoconfig ./gaia-email/autoconfig
	cp -r $(GAIA_DIR)/apps/email/js ./gaia-email/js
	cp -r $(GAIA_DIR)/apps/email/locales ./gaia-email/locales
	cp -r $(GAIA_DIR)/apps/email/sounds ./gaia-email/sounds
	cp -r $(GAIA_DIR)/apps/email/style ./gaia-email/style
